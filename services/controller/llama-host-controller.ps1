...
function Open-HostFolder([string]$requestedPath) {
    ...
    try {
        $explorerProc = Get-Process -Name explorer -ErrorAction SilentlyContinue | Where-Object { $_.SessionId -eq $shellSession } | Select-Object -First 1
        if ($explorerProc) {
            ...
            try {
                ...
                if ($procHandle -ne [IntPtr]::Zero) {
                    ...
                    if ([ProcessHelper]::OpenProcessToken($procHandle, $tokenAccess, [ref]$tokenHandle)) {
                        ...
                        $cmd = '"C:\Windows\explorer.exe" "' + $full.Replace('"', '""') + '"'
                        if ([ProcessHelper]::CreateProcessAsUser($tokenHandle, $null, $cmd, [IntPtr]::Zero, [IntPtr]::Zero, $false, 0, [IntPtr]::Zero, $null, [ref]$si, [ref]$pi)) {
                            ...
                            Write-DebugLog "open-folder path=$full mode=user-session session=$shellSession pid=$($pi.dwProcessId)"
        ...
        Write-DebugLog "open-folder explorer CreateProcessAsUser échec, repli Start-Process" 'warn'
        $proc = Start-Process -FilePath 'explorer.exe' -ArgumentList ('"{0}"' -f $full) -PassThru -ErrorAction Stop
        Start-Sleep -Milliseconds 600
        $explorer = Get-Process -Id $proc.Id -ErrorAction SilentlyContinue
        if ($explorer -and $explorer.MainWindowHandle -ne [IntPtr]::Zero) {
            [ProcessHelper]::ShowWindow($explorer.MainWindowHandle, [ProcessHelper]::SW_RESTORE) | Out-Null
            [ProcessHelper]::SetForegroundWindow($explorer.MainWindowHandle) | Out-Null
        }
    ...
    try {
        ...
        switch ("$($request.method) $path") {
            ...
            'POST /open-folder' {
                ...
                try {
                    $folderResult = Open-HostFolder $target
                    Write-Host "[controller] POST /open-folder path=$target ok=$($folderResult.ok) mode=$($folderResult.mode)"
                    Write-Json $stream 200 $folderResult
                } catch {
                    Write-Host "[controller] POST /open-folder path=$target error=$($_.Exception.Message)"
                    Write-Json $stream 403 @{ ok = $false; path = $target; mode = 'refused'; message = $_.Exception.Message }
                }
                continue
            }
            'POST /start' {
